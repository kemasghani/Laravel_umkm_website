                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body table-responsive">
                            ...
                            <div class="mb-3">
                                <a href="{{ route('admin.seleksi.kriteria.create') }}" class="btn btn-primary">Tambah Data</a>
                            </div>
                            ...
                            <table id="dataTable" class="table table-striped" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($kriteria as $row)
                                        <tr>
                                            <td></td>
                                            <td>{{ $row->aspek->nama_aspek }}</td>
                                            <td>{{ $row->kode_kriteria }}</td>
                                            <td>{{ $row->nama_kriteria }}</td>
                                            <td>{{ $row->jenis == 'cf' ? 'Core Factor' : 'Secondary Factor' }}</td>
                                            <td>{{ $row->target }}</td>
                                            <td>
                                                <form action="{{ route('admin.seleksi.kriteria.destroy', $row->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('admin.seleksi.kriteria.edit', $row->id) }}" class="btn btn-success btn-sm" title="Ubah"><i class="bi bi-pencil"></i></a>
