                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body table-responsive">
                            ...
                            <div class="mb-3">
                                <a href="{{ route('admin.seleksi.subkriteria.create') }}" class="btn btn-primary">Tambah Data</a>
                            </div>
                            ...
                            <table id="dataTable" class="table table-striped" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($subkriteria as $row)
                                        <tr>
                                            <td></td>
                                            <td>{{ $row->kriteria->kode_kriteria }} - {{ $row->kriteria->nama_kriteria }}</td>
                                            <td>{{ $row->nama_subkriteria }}</td>
                                            <td>{{ $row->nilai_bobot }}</td>
                                            <td>
                                                <form action="{{ route('admin.seleksi.subkriteria.destroy', $row->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('admin.seleksi.subkriteria.edit', $row->id) }}" class="btn btn-success btn-sm" title="Ubah"><i class="bi bi-pencil"></i></a>
