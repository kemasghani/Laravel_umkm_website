    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">Berkas UMKM</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <script>
      $(document).ready(function() {
        ...
        $('#dataTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('admin.umkm.index-document')}}",
            columns: [
                    {
                    ...
                    { data: 'download_ktp', name: 'download_ktp',orderable: false, searchable: false},
                    { data: 'download_tempat', name: 'download_tempat',orderable: false, searchable: false},
                    { data: 'download_nib', name: 'download_nib',orderable: false, searchable: false},
      ...
      function editFunc(id){
          $.ajax({
            type:"POST",
            url: "{{ route('admin.umkm.edit-document')}}",
            data: { 
              id: id,
               _token : "{{csrf_token()}}"
            },
      ...
      function hapusFunc(id){
        Swal.fire({
          ...
          if (result.value) {
            $.ajax({
              type:"POST",
              url: "{{ route('admin.umkm.destroy-document')}}",
              data: { 
                id: id,
                 _token : "{{csrf_token()}}"
              },
      ...
      function editStatus(id){
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            $.ajax({
              type:"POST",
              url: "{{ route('admin.umkm.update-status-document')}}",
              data: { 
                id: id,
                _token : "{{csrf_token()}}"
              },
      ...
      $('#upload-image-form').submit(function(e)
      {
        ...
        $.ajax({
            type:'POST',
            url: "{{route('admin.umkm.update-document')}}",
            data: formData,
            contentType: false,
            processData: false,
            success: (response) => {
