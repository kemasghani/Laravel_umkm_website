...
class DocumentController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Request()->ajax()){
            $data = Document::with(['umkms'])->where('status',2)->orWhere('status',3)->get();
            $data_new = array();
            foreach($data as $row){
            ...
            // return $data_new;
             return datatables()->of($data_new)
                ...
                ->addColumn('download_nib',function($data){
                    if($data->nib){
                        $button = "<a href='".route('admin.umkm.download_nib',$data->id)."' class='btn btn-primary btn-sm' data-bs-toggle='tooltip' data-bs-placement='top' title='Download'>
                                    <i class='bi bi-download'></i> Download
                                    </a>";
                    }else{
